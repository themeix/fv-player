...
class Vimeo
{
    ...
    public function request($url, $params = array(), $method = 'GET', $json_body = true, array $headers = array()): array
    {
        ...
        // Somehow when using /videos/{video ID}?fields=uri,name,download,files
        // the presence of the Accept header can cause the files to not be present in the response!
        // But at the same time Accept header is necessary for /me/videos requests to get the video details (names, images)
        if( !preg_match( '~\?fields=.*?files~', $url ) && !preg_match( '~\?fields=.*?download~', $url ) ) {
