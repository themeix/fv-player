 ...
 */
class FV_Player_Pro_Foliopress_Plugin_Private
{
  ...
  private function check_license_remote( $args = array() ) {
    ...
    $defaults = apply_filters( 'fp_api_license_check', $defaults );
...
*/
  ...
  function changelog_filter( $res, $action, $args ){
    ...
    if( !empty($args->fv_readme_file) ){
      $data = file_get_contents($args->fv_readme_file);
    } else if( !empty($this->readme_URL) ) {
      $data = $this->get_readme_url_remote( $this->readme_URL );
    } else {
    ...
    $plugin_data = get_plugin_data(WP_PLUGIN_DIR.'/'.$this->strPluginPath);
