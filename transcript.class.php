...
class FV_Player_Pro_Transcript {
  ...
  function __construct() {
    ...
    add_filter( 'fv_flowplayer_html', array( $this, 'transcript_html' ), 11, 2 );
...
//var_dump($transcript_video_arr);die();
    foreach( $transcript_video_arr as $video_id => $meta_values ) {
      ...
      if ( !empty($transcriptHtml) ) {
        $transcript_html .= '<div ' . $hide . ' class="fv_fp_transcript" id="fv_fp_transcript_' . $element_id . '" data-id="'.$video_id.'" data-player="#wpfp_'.$element_id.'">' . $transcriptHtml . '</div>';
      }
      $first_item = false;
