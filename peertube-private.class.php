...
class FV_Player_Pro_Peertube_Private extends FV_Player_Pro_Ajax_Loader {
  ...
  function generate_tokens($force = false) {
    if( (defined('DOING_AJAX') && DOING_AJAX && !empty($_POST['url']) && !empty($_POST['username']) && !empty($_POST['password'])) || $force ) {
      ...
      $response = wp_remote_get($tokens_url);
  ...
  function options() {
    ...
    if( !empty( $options ) ) {
      ...
      $peertube_private_token_created = isset($options['peertube_private_token_created']) ? $options['peertube_private_token_created'] : '';
      $peertube_private_token_expires = isset($options['peertube_private_token_expires']) ? $options['peertube_private_token_expires'] : '';
    ...
    <table id="fv_player_peertube_new_token" class="form-table2" style="margin: 5px; <?php if ( $configured ) echo 'display: none'; ?>">
      ...
      <tr>
        ...
        <td>
          ...
          <a class="button" href="#" id="peertube-get-tokens" >Generate tokens</a>
        </td>
      </tr>
    </table>
