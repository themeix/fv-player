...
//    add_action( 'wp_ajax_fv_fp_get_video_url', array( $this, 'ajax__get_video_url' ) );
    ...
    add_filter( 'fv_flowplayer_html', array( $this, 'chapters_html' ), 10, 2 );
    ...
    add_action('fv_player_youtube_inputs_after', array( $this, 'fv_player_admin_pro_youtube' ) );
...
<?php
  ...
  function fv_player_admin_pro_youtube() {
    ?>
    <tr>
    ...
    <?php if( $this->_get_option( array('pro','youtube_titles_disable') ) ) $this->_get_checkbox(__('Disable video captions', 'fv-player-pro'), array('pro', 'youtube_titles_disable'), __('Normally the video title is parsed into the shortcode when saving the post, with this setting it won\'t appear.', 'fv-player-pro') ); ?>
    <?php $this->_get_checkbox(__("Disable YouTube <a href='#'>Video Ads</a> on mobile", 'fv-player-pro'), array('pro', 'youtube_ads_disable'), __("Using FV Player Pro Video Ads for YouTube only works for iOS >= 10 and Android >= 6.", 'fv-player-pro'), __("Use this checkbox to disable it completely for all mobile phones if you get any complaints.", 'fv-player-pro') ); ?>
    <script>
  ...
  function is_youtube( $sURL ) {
    if(
      ...
      ) && !$this->_get_option( array('pro','youtube_disable') )
    ) {
      $this->bYoutube = true;
      return $aDynamic;
    }
  ...
  function fetch_vimeo_yt_data($video, $post_id = false, $videoObj = false) {
    ...
    if ( FV_Player_Pro_Vimeo()->is_vimeo($video) && $this->get__vimeo_key() && function_exists('curl_init') ) {
      ...
      if( !$fv_flowplayer_meta ) {
        ...
        $fv_flowplayer_meta['splash'] = str_replace( '?r=pad', '', $fv_flowplayer_meta['splash'] );
    ...
    // TODO: Should be deprecated, it has been moved to free FV Player
    } else if ( ! class_exists( 'FV_Player_YouTube' ) && $this->is_youtube($video) && $this->_get_option( array('pro','youtube_key') )) {
      ...
      if( !$fv_flowplayer_meta ) {
        ...
        $api_url = add_query_arg( array(
          ...
          'id' => $aId[1],
          'key' => $this->_get_option( array('pro','youtube_key') ),
  ...
  function set_file_type( $type ) {
    ...
    if( isset($args[1]) ) {
      if( $this->is_youtube($args[1]) ) {
        $type = "video/youtube";
      } else if( FV_Player_Pro_Vimeo()->is_vimeo($args[1]) || FV_Player_Pro_Vimeo()->is_vimeo_event($args[1]) || $this->is_wistia($args[1]) ) {
        $type = "video/mp4";
