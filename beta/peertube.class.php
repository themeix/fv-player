...
class FV_Player_Pro_Peertube extends FV_Player_Pro_Ajax_Loader {
  ...
  function get_subtitles($url, $securityKey = false) {
    ...
    $domain_root = str_replace('/w/','/', $domain);
    $api_url = $domain_root . 'api/v1/videos/' . $video_id . '/captions';
      ...
      if( isset($captions['data']) && is_array($captions['data']) ) {
        foreach( $captions['data'] as $caption ) {
          $new_caption = array(
            ...
            'src'     => rtrim( $domain_root, '/' ) . $caption['captionPath'],
  ...
  function get_timeline_previews($url, $securityKey = false) {
    ...
    $domain_root = str_replace('/w/','/', $domain);
    $api_url = $domain_root . 'api/v1/videos/' . $video_id . '/storyboards';
    ...
    if( !is_wp_error($response) ) {
      ...
      if( isset( $timeline_previews['storyboards']) && is_array( $timeline_previews['storyboards']) ) {
        foreach( $timeline_previews['storyboards'] as $caption ) {
          $new_caption = array(
            'src' => rtrim( $domain_root, '/' ) . $caption['storyboardPath'],
            'sprite_conf' => array(
