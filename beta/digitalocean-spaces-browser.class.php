...
class FV_Player_DigitalOcean_Spaces_Browser extends FV_Player_Media_Browser {
  ...
  function init() {
    if( $this->isSetUpCorrectly() ) {
      wp_enqueue_script( 'fv-player-dos-browser', plugins_url( 'js/digitalocean-spaces-browser.js', __FILE__ ), array( 'flowplayer-browser-base' ), FV_Player_Pro()->version );
    }
  }
  ...
  function get_formatted_assets_data() {
    ...
    global $fv_fp, $s3Client, $FV_Player_Pro_DigitalOcean_Spaces;

    $endpoint = 'https://' . $FV_Player_Pro_DigitalOcean_Spaces->get_endpoint();

    $region = $FV_Player_Pro_DigitalOcean_Spaces->get_region();
