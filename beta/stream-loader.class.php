  ...
  class FV_Player_Pro_Stream_Loader {
    ...
    function options() {
      ...
      <table class="form-table2" style="margin: 5px; ">
        ...
        <?php $fv_fp->_get_checkbox(__('Speed-up', 'fv-player-pro').' (beta)', array('pro', 'stream_loader_speed_up'), __('Activate for faster serving.', 'fv-player-pro'), sprintf( __('Will bypass WordPress and use <code>%s</code> URL directly. Unfortunately the email notifications on HTTP failures when loading HLS will be disabled.', 'fv-player-pro'), plugins_url( 'stream-loader.php', __FILE__ ) ) ); ?>
        <?php if ( ! FV_Player_Pro()->_get_option( array('pro','stream_loader_speed_up') ) ) {
          $fv_fp->_get_checkbox(__('Emulate m3u8 files', 'fv-player-pro').' (beta)', array('pro', 'stream_loader_m3u8'), __('Activate if playback on iOS fails.', 'fv-player-pro'), __('Some web servers refuse to accept HTTP range requests for PHP scripts. Enable this setting if you noticed your HLS videos do not play on iPhone and every playback will use the <code>/index.m3u8</code> URL with parameters.', 'fv-player-pro') );
        } ?>
    ...
    */
    ...
    function stream_loader_url($video_id, $sub_playlist_id = -1, $non_recursive = false, $cache_expire_override = 0, $no_protection = false ) {
      ...
      } else if ( FV_Player_Pro()->_get_option( array('pro', 'stream_loader_m3u8') ) ) {
        $url = untrailingslashit( home_url( 'index.m3u8' ) );
