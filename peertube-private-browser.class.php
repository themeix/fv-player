...
class FV_Player_Pro_Peertube_Private_Browser extends FV_Player_Media_Browser {
  ...
  function init() {
    if( $this->isSetUpCorrectly() ) {
      wp_enqueue_script( 'fv-player-peertube-private-browser', plugins_url( 'js/peertube-private-browser.js', __FILE__ ), array( 'flowplayer-browser-base' ), FV_Player_Pro()->version );
  ...
  function get_formatted_assets_data() {
    ...
    try {
      ...
      $api_url = $url . '/api/v1/users/me/videos'; // get vides from current user
